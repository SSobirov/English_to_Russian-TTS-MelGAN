...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    ...
    local/download.sh ${db_root} ru_RU
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # Russian
    for spk in minaev nikolaev; do
        echo "Processing ${spk}..."
        local/data_prep.sh ${db_root} data/${spk} ru_RU ${spk} ${trans_type}
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    fbankdir=fbank
    for x in judy elliot minaev nikolaev; do
